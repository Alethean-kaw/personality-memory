...
class MemoryConsolidator:
            ...
            if best_match is not None:
                ...
                if contradiction >= self.contradiction_threshold:
                    ...
                    self._resolve_candidate(
                        candidate,
                        status="review",
                        notes=f"Potential contradiction with {best_match.id}",
                        resolution_kind="conflict",
                        resolved_memory_id=best_match.id,
                    )
                    self._open_review_item(
                        ...
                        best_match.id,
                        kind="conflict",
                        reason=f"Candidate {candidate.id} may contradict memory {best_match.id}.",
                    )
                    conflicts += 1
                    if best_match.mutable:
                        revisions.append(
                ...
                if best_similarity >= self.similarity_threshold:
                    if not best_match.mutable:
                        self._resolve_candidate(
                            candidate,
                            status="accepted",
                            notes=f"Matched immutable memory {best_match.id}; no auto-merge",
                            resolution_kind="matched_immutable",
                            resolved_memory_id=best_match.id,
                        )
                        continue
                    ...
                    self._resolve_candidate(
                        candidate,
                        status="accepted",
                        notes=f"Merged into {best_match.id}",
            ...
            if inactive_match is not None and inactive_similarity >= self.similarity_threshold:
                if not inactive_match.mutable:
                    self._resolve_candidate(
                        candidate,
                        status="accepted",
                        notes=f"Matched immutable inactive memory {inactive_match.id}; no auto-merge",
                        resolution_kind="matched_immutable",
                        resolved_memory_id=inactive_match.id,
                    )
                    continue
                ...
                self._resolve_candidate(
                    candidate,
                    status="accepted",
                    notes=f"Revived and merged into {inactive_match.id}",
                    resolution_kind="merged",
                    resolved_memory_id=inactive_match.id,
                )
                updated += 1
                revisions.append(
                    RevisionEntry(
                        id=f"rev_{stable_hash(f'revive|{inactive_match.id}|{candidate.id}|{utc_now()}')}",
                        entity_type="long_term_memory",
                        entity_id=inactive_match.id,
                        action="revive",
                        timestamp=utc_now(),
                        reason=f"Candidate {candidate.id} revived memory {inactive_match.id}",
                        before=before,
                        after=inactive_match.to_dict(),
                    )
                )
                continue

            support_count = self._compute_candidate_support(candidates).get(candidate.id, 1)
    ...
    def _default_reference_time(self, candidates: list[MemoryCandidate], memories: list[LongTermMemory]) -> str:
        ...
        timestamps.extend(
            memory.last_reinforced_at or memory.last_seen or memory.first_seen
            for memory in memories
            if (memory.last_reinforced_at or memory.last_seen or memory.first_seen)
        )
    ...
    def _compute_candidate_support(self, candidates: list[MemoryCandidate]) -> dict[str, int]:
        pending_candidates = [candidate for candidate in candidates if candidate.status == "candidate"]
        support = {candidate.id: 1 for candidate in pending_candidates}
        for index, candidate in enumerate(pending_candidates):
            for other in pending_candidates[index + 1 :]:
                if candidate.type != other.type:
                    continue
                score = self.backend.similarity(candidate.content, other.content) + 0.03
        ...
        *,
        ...
        for item in review_items:
            if (
                ...
                and item.target_memory_id == target_memory_id
                and item.kind == kind
            ):
                item.reason = reason
                return item
        ...
        review_item = ReviewItem(
            id=f"review_{stable_hash(f'{candidate.id}|{target_memory_id}|{kind}|{utc_now()}')}",
